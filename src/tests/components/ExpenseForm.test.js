...
test('should set note on text area change', () => {
    const wrapper = shallow(<ExpenseForm />)
    const value = 'This is a note that will be written in the note area'
...
test('should set amount on valid input change', () => {
    const wrapper = shallow(<ExpenseForm />)
    const value = '123.45'
    wrapper.find('input').at(1).simulate('change', { target: { value: value}})
...
test('should set calendard focus on focus change', () => {
    const focused = true
    const wrapper = shallow(<ExpenseForm />)
